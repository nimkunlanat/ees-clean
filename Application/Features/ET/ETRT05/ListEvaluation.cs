...
public class ListEvaluation
{
    ...
    public class Handler : IRequestHandler<Query, List<EvaluateGroup>>
    {
        ...
        public async Task<List<EvaluateGroup>> Handle(Query request, CancellationToken cancellationToken)
        {
            ...
            if (request.Keywords != null) sql.AppendLine(@"and concat(eg.evaluate_group_code, eg.evaluate_group_name_th, eg.evaluate_group_name_en, eg.total_point) ilike concat('%',@Keywords,'%')");

            sql.AppendLine("order by eg.sequene_id");

            return await _context.QueryAsync<EvaluateGroup>(sql.ToString(), new { Keywords = request.Keywords, RoleCode = request.RoleCode }, cancellationToken) as List<EvaluateGroup>;
        }
    }
}
